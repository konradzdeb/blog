...
* Running an update for outdated packages every so often. This is relatively good habit, comparable to running `brew upgrade` with some frequency so we can avoid our production environment differentiating more from the current stable releases.
...
As I want for this function to install only to a specific library and from a specific repository I'm using the `--vanilla` switch to let Rscript know that I don't need it to process `.RProfile` and other files.
