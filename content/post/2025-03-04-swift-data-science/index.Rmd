...
knitr::knit_engines$set(swift = function(options) {
  ...
  # Preceding chunks
  prior_chunk_names <- swift_chunk_names[seq_len(Position( \(x) x == knitr::opts_current$get("label"), swift_chunk_names ))]
  # All Swift code
  collected_swift_code <- Reduce( \(x, y) { paste(x, knitr::knit_code$get(y), sep = "\n") }, prior_chunk_names, init = "")
  # Filter Swift code
  filtered_swift_code <- Filter( \(x) { !grepl(".*print.*", x) }, collected_swift_code)
  # Run the collected Swift code
  out <- system2(command = "swift", args = "repl", input = filtered_swift_code, stdout = TRUE, stderr = TRUE)
...
```{swift, hello_two, eval=TRUE, include=TRUE}
