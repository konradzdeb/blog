...
In  context of importing wide tables this will prove particularly painful. If we are in position where we are at a risk of undertaking that type of exercise more frequently, reflecting on a process that would enable us to automate or part-automate the data load can prove beneficial. 
...
Before uploading the data we devise a table structure that will be used to hold it. Following the displayed variable types this can be done in the following manner. From a development perspective, breaking functions into manageable smaller pieces will make it easier to develop.  
...
We can assume a more rigours approach reflecting the preference that, in all likelihood, majority of us would endorse:
...
- With exception of the three letter currency code, the column name should be in lowercase. Hive is case-insensitive but pedentically, I would prefer for the column to be named `price_GBP` instead of `price_gbp`. As this is matter of preference we may make this operation optional
...
#'   defaults to commonly occurring words in column name like "per, by, for"
#'
