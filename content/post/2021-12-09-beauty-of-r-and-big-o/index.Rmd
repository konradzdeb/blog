...
|----------------------+--------------------------------------------------------------+--------------+---------------------------------------------------------------|
| Big-O                | Equivalent to $\le$, growth less or equal specifc value      | Worst case   | Longest amount of time it could take to algorithm to complete |
| Big-Omega $(\Omega)$ | Equivalent to $\ge$, growrth greater or equal specific value | Best case    | Compared to Big-O, reflects smaller amount of time            |
...
So I wanted to compare a speed of some algorithms and reflecting a few common curves, say $n*\log(n)$, $\log(n)$, $n^2$ and so forth. I need a quick visual aid where I could plot some most common running times and add a few to quickly eyeball how they faire against the algorithm I was working on. 

## Functions

So I've defined a few functions that correspond to the basic Big-O curves that can be found in majority of introductory literature on the subject with the intention of quickly producing visual base for the subject.
...
```{r full_solution}
n <- 1e2 # This is a bad habit of mine but while in R, I've a strong preference for using 1e6 for million, 1e2 for hundred and so forth
...
In a production setting, we probably wouldn't rely on `ls(pattern = ...)` and `mget` sourcing objects by name. There even with setting up `mode` argument and limiting results to function there is a risk that our code could capture an undesired call that from one or another reason could appear in the environment where `ls` would happen to be executing search. Finally for quickly plotting trivial functions, like $y=log(x)^2$ can be easily achieved with use of dedicated software. Mac users will already have [Grapher](https://support.apple.com/en-gb/guide/grapher/gcalb3dec608/mac) installed that meets that requirement.

Nevertheless, ability to treat functions as objects is exceptionally useful, especially if we need to iterate through a number of function, and quickly compare the results on a set of data.
