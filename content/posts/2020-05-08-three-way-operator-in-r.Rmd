...
```{r three-way-def, eval=FALSE, echo=TRUE}
(a <=> b) < 0  # if lhs < rhs
(a <=> b) > 0  # if lhs > rhs
...
```{r example-ifelse, eval=TRUE, echo=TRUE}
a <- 1
b <- 2
ifelse(a < b, -1, 1)
```
...
```{r example-ifelse-even, eval=TRUE, echo=TRUE}
a <- 1
b <- 1
ifelse(a < b, -1, 1)
```
...
```{r example-ifelse-even-working, eval=TRUE, echo=TRUE}
a <- 1
b <- 1
...
```{r wrong-types, echo=TRUE, eval=TRUE}
a <- "a"
b <- 1
...
```{r sample_plus, echo=TRUE}
5 + 2
```

statement is equivalent to

```{r plus_infix, echo=TRUE}
`+`(5,2)
```
